    ...
    <div class="status-group">
      ...
      <transition name="slide-fade-input">
        <div v-if="strategy === '自定义'" class="custom-input-wrapper">
          <input
            type="text"
            class="custom-strategy-input"
            :value="customContent"
            @change="onCustomContentChange"
            placeholder="输入你的战斗策略..."
            title="按回车或失去焦点保存"
          />
        </div>
      </transition>
...
const onStrategyChange = (e: Event) => {
  ...
  emit('update:strategy', target.value);
};

const onCustomContentChange = (e: Event) => {
  const target = e.target as HTMLInputElement;
  emit('update:customContent', target.value);
};
</script>
