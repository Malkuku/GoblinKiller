...
const getMessagesByRange = (messageId: number, index: number) => {
  const recordMessages = getChatMessages(
    `${messageId - index >= 0 ? messageId - index : 0}-${messageId}`,
  );
  let records = '';
