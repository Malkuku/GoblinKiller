...
.text-body :deep(q) {
  ...
  /* 默认状态：极细的半透明虚线，贴近文字 */
  text-decoration: underline dashed rgba(198, 166, 100, 0.4);
  text-underline-offset: 2px; /* 距离文字较近 */
  text-decoration-thickness: 1px; /* 线条极细 */
...
.text-body :deep(q) {
  ...
  /* 默认状态：降低高亮高度，透明区域占75%，底部25%为淡淡的金色 */
  background: linear-gradient(to bottom, transparent 75%, rgba(198, 166, 100, 0.25) 75%);
  border-radius: 2px;

  /* 提前设置透明的下划线，防止hover时文字发生抖动 */
...
.text-body :deep(q:hover) {
  /* 悬浮状态：褪去水彩背景 */
  background: linear-gradient(to bottom, transparent 100%, rgba(198, 166, 100, 0) 100%);

  /* 显现出向下偏移的深色平虚线 */
  text-decoration-color: var(--accent-gold, #c6a664);
}
...
.text-body :deep(.is-thought) {
  ...
  border-left: 3px solid rgba(198, 166, 100, 0.3); /* 将虚线改为柔和的实线，更显高级 */
}
